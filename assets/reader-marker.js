(() => {
  ...
  function updateContinuationCue() { if (!cue) return; const last = atLastPlace(); cue.classList.toggle('reader-can-continue', Boolean(last && nextHref)); if (last && nextHref) { cue.textContent = 'Continue'; cue.setAttribute('role', 'link'); cue.setAttribute('tabindex', '0'); cue.setAttribute('aria-label', 'Continue to the next episode'); } else { cue.textContent = last ? 'Living edge' : 'Swipe for more'; cue.removeAttribute('role'); cue.removeAttribute('tabindex'); cue.removeAttribute('aria-label'); } }
